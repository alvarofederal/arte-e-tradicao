...
const EMAIL_FROM =
  process.env.EMAIL_FROM ?? "Arte & Tradição <noreply@arteetradicao.com.br>"
...
export async function sendVerificationEmail(
  ...
  const { error } = await getResend().emails.send({
    from: EMAIL_FROM,
    to: email,
    subject: "🔐 Código de Verificação - Arte & Tradição",
    html: emailHtml,
  })
...
<html>
  ...
  <body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;margin:0;padding:0;background:#f4f4f4">
    <div style="max-width:600px;margin:40px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 6px rgba(0,0,0,.1)">
      ...
      <div style="background:#f9fafb;padding:30px;text-align:center">
        <p style="color:#6b7280;font-size:14px;margin:0">© ${new Date().getFullYear()} Arte & Tradição. Todos os direitos reservados.</p>
      </div>
    </div>
  </body>
</html>`
  ...
  const { error } = await getResend().emails.send({
    from: EMAIL_FROM,
    to: email,
    subject: "🔑 Recuperação de Senha - Arte & Tradição",
    html: emailHtml,
  })
